...
export const MAN_PAGES = {
  ...
  awk:    { synopsis:"awk [-F FS] [-v VAR=VALUE] 'プログラム' [FILE...]", desc:"入力をフィールド単位に分割し、パターンとアクションの組み合わせで集計・レポート作成を行うテキスト処理言語。既定のフィールド区切りは空白文字。", options:[
    ["-F FS","フィールド区切り文字（1文字または正規表現）を指定する"],
    ["-v VAR=VALUE","プログラム実行前に変数VARにVALUEを設定する"],
    ["$0 / $1,$2…","$0は行全体、$1以降は各フィールドを参照する"],
    ["NR / NF","NRは現在の行番号、NFは現在の行のフィールド数を表す組み込み変数"],
