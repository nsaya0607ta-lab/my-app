...
export const MAN_PAGES = {
  ...
  kill:   { synopsis:"kill [-9] PID", desc:"指定したPIDのプロセスにシグナルを送る。既定はSIGTERM(15)。反応しない場合は -9（SIGKILL）で強制終了する。" },
  killall:{ synopsis:"killall [-9] NAME", desc:"プロセス名を指定して該当する全プロセスにシグナルを送る。" },
  sleep:  { synopsis:"sleep SECONDS", desc:"指定した秒数だけ処理を停止する。時間のかかる処理を試すときに使う。末尾に & を付けるとバックグラウンドで実行できる。" },
  jobs:   { synopsis:"jobs", desc:"現在のシェルが持つジョブ（&でバックグラウンド実行した処理やCtrl+Zで停止した処理）の一覧を表示する。" },
  fg:     { synopsis:"fg [%N]", desc:"バックグラウンドまたは停止中のジョブをフォアグラウンドに戻す。番号省略時は直近のジョブが対象になる。" },
  bg:     { synopsis:"bg [%N]", desc:"Ctrl+Zなどで停止中のジョブを、バックグラウンドで動かしたまま再開する。" },
  ...
  lsblk:  { synopsis:"lsblk", desc:"接続されているブロックデバイス（ディスク）とパーティション、マウント先を一覧表示する。" },
  fdisk:  { synopsis:"fdisk [-l] [デバイス名]", desc:"ディスクのパーティションテーブルを操作する。デバイス名を指定して実行すると対話式のパーティション編集画面が開く（root権限が必要）。", options:[
    ["-l","対話式に入らず、パーティションテーブルの内容だけを表示する"],
  ]},
