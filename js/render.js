...
// series: 日足（Daily）の {t: 時刻(ms), close: 終値} 配列。X軸=日付・
// Y軸=株価としてチャートにそのまま使う。モックは start→end へゆるやかに
// ランダムウォークする擬似的な値動きを、1日間隔で30ポイント生成する。
...
function stocksCardHTML(){
  return `
    <div class="news-card stocks-card" id="stocks-card">
      ...
      <div class="stock-period" id="stock-chart-period">日足 (Daily)</div>
    </div>`;
}
...
// 日足（Daily）チャートを外部CDNに頼らず純粋なSVGで自前描画する。
// Chart.js（CDN経由）は実機のネットワーク環境によって読み込みに失敗し、
// チャートが空白のままになる不具合があったため、外部依存のないこの方式を
// 採用している。X軸=日付／Y軸=株価、タップ/ドラッグで該当日の日付・
// 価格をツールチップ表示する。
function renderStockChart(){
  const periodEl = document.getElementById("stock-chart-period");
  const s = STOCKS[stockIndex];
  if(periodEl){
    periodEl.textContent = s.chartIsLive ? "日足 (Daily)" : (s.everLive ? "日足 (Daily・最終参照)" : "日足 (Daily・サンプル)");
  }
  ...
  if(xAxisEl){
    ...
    xAxisEl.innerHTML = idxs.map(i => `<span>${formatChartDate(series[i].t)}</span>`).join("");
  }
  ...
  function showAt(clientX){
    ...
    dot.style.display = "";
    tip.style.display = "";
    tip.textContent = `${formatChartDate(p.t)}  $${p.close.toFixed(2)}`;
...
// 日足チャートなので新しい日を追加することはせず、直近（今日）の終値ポイントだけを
// 動かして表示を維持する。
