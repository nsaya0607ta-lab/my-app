...
function buildSystemInstruction(today, appContext, plainText){
  const lines = [
    ...
    ...(plainText ? [
      "【表示形式（最優先で厳守）】回答には*（強調・箇条書き記号）や#（見出し記号）などのMarkdown記号を一切使わないでください。強調したい語も** **のような記号で囲まず、そのまま文章中に書いてください。見出しが欲しくなる場面でも#は使わず、代わりに「◆」等の記号か、行を分けた通常の文章で表現してください。箇条書きが必要な場合は必ず「・」を行頭に使ってください。ただし、後述するコマンド実行例の```コードブロックはこの制限の対象外なので、そちらは通常どおり```で囲んでください。",
    ] : []),
    "【コマンドの実行例（最優先で厳守）】ユーザーが特定のLinux/UNIXコマンド（例：ls、grep、chmod、find、systemctlなど）の使い方・書式・オプション・実行結果について質問した場合は、「LPICの問題」と明示されていなくてもコマンド名が具体的に含まれていれば対象です。文章による説明だけで終わらせず、必ずそのコマンドの実行例を```で囲んだコードブロックとして示してください。書式は次の例を一字一句そのまま真似てください（```の位置・1行目のプロンプト記号「$ 」・2行目以降はプロンプト記号なしの結果、を厳守すること）：\n```\n$ ls -la /home/user\ntotal 24\ndrwxr-xr-x 5 user user 4096 Jul 16 07:00 .\ndrwxr-xr-x 3 root root 4096 Jul  1 09:00 ..\n-rw-r--r-- 1 user user  220 Jul  1 09:00 .bash_logout\n```\n（実際の結果部分は、存在しそうなファイル名・数値等でそのコマンドらしい出力を作成してください。）複数のコマンド例を1つの回答で示したい場合は、コマンドと結果の組ごとに```コードブロックを分けてください。このコードブロックはアプリ側が自動で黒いターミナル風の見た目に整形し、コマンド部分にはコピー用ボタンも付くので、あなたはこの```記法さえ守れば他の装飾を追加する必要はありません。",
