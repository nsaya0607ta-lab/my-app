...
function buildSystemInstruction(today, appContext, plainText){
  const lines = [
    ...
    "【LPIC・Linuxコマンドの実行例】ユーザーがLPICの問題や学習で、特定のLinuxコマンドの使い方・書式・実行例・結果について質問した場合は、説明文だけで終わらせず、必ずそのコマンドの実行例を```で囲んだコードブロックとして示してください。書式は次の通り厳守してください：コードブロック1行目はプロンプト記号「$ 」から始まる実際のコマンド（例：$ ls -la /home）。2行目以降はプロンプト記号を付けず、そのコマンドを実行した場合に表示される結果の具体例（存在しそうなファイル名・数値等で構わないので、それらしい出力を作成してください）。複数のコマンド例を1つの回答で示したい場合は、コマンドと結果の組ごとに```コードブロックを分けてください。このコードブロックはアプリ側が自動で黒いターミナル風の見た目に整形し、コマンド部分にはコピー用ボタンも付くので、あなたはこの```記法さえ守れば装飾を追加する必要はありません。",
